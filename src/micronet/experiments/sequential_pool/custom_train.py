...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        min_tensor = tf.constant(tf.float32.min, tf.float32,
                                 shape=(q_values.shape[-1],))
        masked_action_vals = tf.compat.v1.where_v2(mask, q_values, min_tensor)
        ...
        random_actions = tf.argmax(tf.compat.v1.where_v2(mask, random_actions, below_zero), axis=1)
