...
def get_ema_vars(scope):
  """Get all exponential moving average (ema) variables."""
  ema_vars = tf.trainable_variables(scope) + tf.get_collection('moving_vars')
  for v in tf.global_variables(scope):
...
class EvalCkptDriver(object):
  ...
  def restore_model(self, sess, ckpt_dir, enable_ema=True, export_ckpt=None):
    ...
    if enable_ema:
      ...
      ema_vars = get_ema_vars(self.model_name)
      var_dict = ema.variables_to_restore(ema_vars)
      var_dict = {k:v for k,v in var_dict.items()
                    if k.startswith(self.model_name)}
      ema_assign_op = ema.apply(ema_vars)
    else:
      var_dict = get_ema_vars(self.model_name)
      ema_assign_op = None
